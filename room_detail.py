...
url = 'https://www.airbnb.com/rooms/1358147?s=aB8d'
...
def get_room_detail(url):
	...
	try:
		...
		nb_guest,nb_bedroom = summary_tag[1].get_text(strip=True),summary_tag[2].get_text(strip=True)
		nb_bed = summary_tag[3].get_text(strip=True)
		...
		room['location'] = soup.select('div#display-address a[href=#neighborhood]')[0].get_text(strip=True)
		...
		if soup.find('meta', attrs={'property': 'airbedandbreakfast:rating', 'content': True}):
		...
		room['url'] = url.split('?',1)[0]
		room['title'] = soup.select('title')[0].get_text(strip=True)
		if soup.select('div.expandable-content.expandable-content-long'):
			room['description'] = soup.select('div.expandable-content.expandable-content-long')[0].get_text(strip=True)
		return room
